...
<head>
 ...
 $("#id").keyup(function() {    // id입력 상자에 id값 입력시
    ...
    $.ajax({  
        type: "POST",           // get, post 사용
        url: "check_id.php",    // check_id.php 로   data를 넘김
        data: "id="+ id,        // 같이 넘길 변수   
        cache: false,           // 정보를 캐시에 기록하느냐(true) 안하느냐(false)  
        success: function(data) // 위 값들의 처리가 완료된다면 
        {
            //data => echo "문자열" 이 저장된
             $("#loadtext").html(data);//loadtext:  실시간바뀌는 span 
        }
    });
});
...
<?
    ...
    $sql = "select * from member where id='$userid'"; // id 검색  select * from ~어디 
    $result = mysql_query($sql, $connect);

    $row = mysql_fetch_array($result);
    //$row[id]....$row[level]

    $hp = explode("-", $row[hp]);   // explode: 문자열을 끊어주는 매소드 이문장에서는 '-' 하이푼 기준
    //000-0000-0000   
    $hp1 = $hp[0]; // 000
    $hp2 = $hp[1]; // 중간자리 0000
    $hp3 = $hp[2]; //  뒷 자리 0000

    $email = explode("@", $row[email]); // explode: 문자열을 끊어주는 매소드 이문장에서는 '@' 골뱅이 기준
    $email1 = $email[0]; // zhuo95
    $email2 = $email[1]; // nate.com

    mysql_close(); //data base 종료
?>
<body>
    ...
    <?= $row[id] ?>           
     	...
     	<tr>
     		...
     		<td>
			     ...
			     <span id="loadtext2"></span> <!-- 실시간 메시지 찍어주는 span -->
     		</td>
     	</tr>
     	<tr>
     		<th scope="col">휴대폰</th>
     		<td>
     			<label class="hidden" for="hp1">전화번호앞3자리</label> <!--수정수정 -->
